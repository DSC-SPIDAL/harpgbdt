...
if [ $# -eq "0" ] ; then
	echo "Usage: lightgbm-speedup.sh <bin> <dataset> <iter> <maxdepth> <tree_method> <thread> <fid>"
    exit -1
fi
...
prefix=$binname-${dataset}-${fid}-n${num_round}-d${max_depth}-m${tree_method}-t${thread}
...
for runid in ${runids[*]}; do
	...
	$bin config=$conf num_trees=${num_round} nthread=${thread} tree_learner=${tree_method} num_leaves=${num_leaves} data=higgs-${fid}.libsvm 2>&1 | tee -a ${logfile}.log
