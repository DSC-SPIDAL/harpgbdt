...
for thread in ${threads[*]}; do
    ...
    rowblksize=$(echo "(10000000 + ${thread} - 1)/${thread}" | bc)

	echo "$bin $conf num_round=${num_round} nthread=${thread} tree_method=${tree_method} max_depth=0 bin_block_size=${bin_blksize} ft_block_size=${ft_blksize} row_block_size=${rowblksize} node_block_size=${node_blksize} grow_policy=lossguide max_leaves=${num_leaves} ${otherargs} " | tee ${logfile}.log
 
	$bin $conf num_round=${num_round} nthread=${thread} tree_method=${tree_method} max_depth=0 bin_block_size=${bin_blksize} ft_block_size=${ft_blksize} row_block_size=${rowblksize} node_block_size=${node_blksize} grow_policy=lossguide max_leaves=${num_leaves}  ${otherargs} group_parallel_cnt=${thread} 2>&1 | tee -a ${logfile}.log
