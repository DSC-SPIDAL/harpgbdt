 ...
 *
 * ft_membuf, add gpair into PosEntry
 *
 */
#ifndef XGBOOST_COMMON_POS_SET_LOSSGUIDE_GPAIR_H_
#define XGBOOST_COMMON_POS_SET_LOSSGUIDE_GPAIR_H_
...
class POSSetSingleGPair{
    ...
    struct POSEntry{
        ...
        POSEntry(int rowid, const GradientPair& gh){
    ...
    struct POSGroup{
        ...
        inline GradientPair& getGHPair(int i){
            return _start[i].getGHPair();
        }
    ...
    public:
    POSSetSingleGPair() = default;


    //void Init(int rownumber, int nodenum, int row_block_size = 0){
    void Init(const std::vector<GradientPair> &gpair,
            int nodenum, int row_block_size = 0){

        int rownumber = gpair.size();
